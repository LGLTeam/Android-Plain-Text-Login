#include <jni.h>
#include <unistd.h>
#include <pthread.h>
#include "Includes/Logger.h"
#include "Includes/Utils.h"
#include "Includes/obfuscate.h"

bool titleValid = false, headingValid = false, iconValid = false, featureListValid = false;
bool featuresEnabled;

void *antiLeech(void *) {
    sleep(15);

    if (!titleValid) {
        //Bad function to make it crash
        //Note: We want to avoid using exit() method call
        int *p = 0;
        *p = 0;
    }
    if (!headingValid) {
        int *p = 0;
        *p = 0;
    }
    if (!iconValid) {
        int *p = 0;
        *p = 0;
    }
    if (!featureListValid) {
        int *p = 0;
        *p = 0;
    }
    return NULL;
}

extern "C" {
JNIEXPORT jboolean
JNICALL
Java_uk_lgl_modmenu_loader_EnableSounds(JNIEnv *env, jobject thiz) {
    //Note: some games don't like sounds, which caused crash. Please turn it off if it happens
    return true;
}

JNIEXPORT jstring
JNICALL
Java_uk_lgl_modmenu_loader_Icon(JNIEnv *env, jobject thiz) {
    iconValid = true;

    //Use https://www.base64encode.org/ to encode your image to base64
    return env->NewStringUTF(
            "");
}

JNIEXPORT jstring
JNICALL
Java_uk_lgl_modmenu_loader_IconWebViewData(JNIEnv *env, jobject thiz) {
    iconValid = true;
    //return env->NewStringUTF("https://i.imgur.com/SujJ85j.gif");
    return NULL;
}

JNIEXPORT jint
JNICALL
Java_uk_lgl_modmenu_loader_IconSize(JNIEnv *env, jobject thiz) {
    return 50;
}

JNIEXPORT jstring
JNICALL
Java_uk_lgl_modmenu_loader_Title(JNIEnv *env, jobject thiz) {
    titleValid = true;
    return env->NewStringUTF(OBFUSCATE("Whatever"));
}

JNIEXPORT jstring
JNICALL
Java_uk_lgl_modmenu_loader_Heading(JNIEnv *env, jobject thiz) {
    headingValid = true;
    return env->NewStringUTF(OBFUSCATE("Mod menu"));
}



JNIEXPORT jstring JNICALL
Java_uk_lgl_modmenu_loader_Sw(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(OBFUSCATE("Switch_"));
}
JNIEXPORT jstring JNICALL
Java_uk_lgl_modmenu_loader_Se(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(OBFUSCATE("SeekBar_"));
}
JNIEXPORT jstring JNICALL
Java_uk_lgl_modmenu_loader_Ca(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(OBFUSCATE("Category_"));
}
JNIEXPORT jstring JNICALL
Java_uk_lgl_modmenu_loader_Bu(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(OBFUSCATE("Button_"));
}
JNIEXPORT jstring JNICALL
Java_uk_lgl_modmenu_loader_Sp(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(OBFUSCATE("Spinner_"));
}
JNIEXPORT jstring JNICALL
Java_uk_lgl_modmenu_loader_In(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(OBFUSCATE("InputValue_"));
}
JNIEXPORT jstring JNICALL
Java_uk_lgl_modmenu_loader_csv(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(OBFUSCATE("#40000000"));
}
JNIEXPORT jstring JNICALL
Java_uk_lgl_modmenu_loader_cbv(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(OBFUSCATE("#40000000"));
}
JNIEXPORT jstring JNICALL
Java_uk_lgl_modmenu_loader_xnxx(JNIEnv *env, jobject thiz) {
    return env->NewStringUTF(OBFUSCATE("X"));
}


JNIEXPORT jobjectArray
JNICALL
Java_uk_lgl_modmenu_loader_getFeatureList(JNIEnv *env, jobject activityObject) {
    jobjectArray ret;
    featureListValid = true;
    const char *features[] = {
            OBFUSCATE("Category_"),
            OBFUSCATE("Switch_Switch 1"),
            OBFUSCATE("Switch_Switch 2"),
            OBFUSCATE("Switch_Switch 3"),
            OBFUSCATE("SeekBar_Seek_0_10"),
            OBFUSCATE("Category_"),
            OBFUSCATE("Category_YouTube: User\nTelegram: User\n\n\nBest cheat\n\nCopyright ©2020 | LGL All Rights Reserved"),
            OBFUSCATE("Category_"),
    };

    int Total_Feature = (sizeof features /
                         sizeof features[0]);

    ret = (jobjectArray)
            env->NewObjectArray(Total_Feature, env->FindClass(OBFUSCATE("java/lang/String")),
                                env->NewStringUTF(""));
    int i;
    for (i = 0; i < Total_Feature; i++)
        env->SetObjectArrayElement(ret, i, env->NewStringUTF(features[i]));

    //Run anti-leech
    pthread_t ptid;
    pthread_create(&ptid, NULL, antiLeech, NULL);
    return (ret);
}
}

